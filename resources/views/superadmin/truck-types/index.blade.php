                    ...
                    <tr>
                        ...
                        <td class="table-actions">
                            <div class="action-buttons">
                                ...
                                <button class="edit-btn" data-id="{{ $type->id }}" data-name="{{ $type->name }}"
                                    data-base="{{ $type->base_rate }}" data-km="{{ $type->per_km_rate }}"
                                    data-tonnage="{{ $type->max_tonnage }}" data-description="{{ $type->description }}"
                                ...
                                @if ($type->status === 'active')
                                    <button onclick="openDisableModal({{ $type->id }}, '{{ $type->name }}')"
                                        class="btn-danger">
                                        <i data-lucide="ban"></i>
                                        <span>Disable</span>
                                    </button>
        ...
        <div class="modal-card">
            ...
            <div class="modal-header">
                <div>
                    ...
                    <p>Create a new truck type with pricing</p>
                </div>
            ...
            <form method="POST" action="{{ route('superadmin.truck-types.store') }}">
                ...
                <div class="form-group">
                    ...
                    <small class="input-hint">Unique name for this vehicle category</small>
                    <input type="text" name="name" placeholder="e.g. Light Duty" required>
                </div>

                <div class="form-row">
                    <div class="form-group">
                        <label>Base Rate</label>
                        <small class="input-hint">Starting price (first 4km included)</small>
                        <input type="number" step="0.01" name="base_rate" placeholder="₱1500" required>
                    </div>

                    <div class="form-group">
                        <label>Per KM Rate</label>
                        <small class="input-hint">Cost per km after base distance</small>
                        <input type="number" step="0.01" name="per_km_rate" placeholder="₱200" required>
                    </div>
                </div>

                <div class="form-group">
                    <label>Max Tonnage</label>
                    <small class="input-hint">Maximum weight capacity</small>
                    <input type="number" step="0.01" name="max_tonnage" placeholder="e.g. 4.5 tons" required>
                </div>

                <div class="form-group">
                    <label>Description</label>
                    <small class="input-hint">Optional short description</small>
                    <textarea name="description" placeholder="Short details about this truck type"></textarea>
                </div>

                <div class="modal-footer">
                    <button type="button" onclick="closeAddModal()" class="btn-light">
                        Cancel
                    </button>
        ...
        <div class="modal-card">
            ...
            <div class="modal-header">
                <div>
                    ...
                    <p>Update pricing and details</p>
                </div>
    ...
    <div id="disableModal" class="modal">
        <div class="modal-content">
            ...
            <div style="margin-bottom: 12px;">
                <i data-lucide="alert-triangle" style="color:#ff4d4f; width:32px; height:32px;"></i>
            </div>
            ...
            <form method="POST" id="disableForm">
                ...
                <div class="modal-actions">
                    <button type="button" onclick="closeDisableModal()" class="btn-cancel">
                        Cancel
                    </button>
...
@push('scripts')
    <script>
        ...
        function openEditModal(button) {
            ...
            document.getElementById('editForm').action = `/superadmin/truck-types/${id}`;
        ...
        function openDisableModal(id, name) {
            document.getElementById('disableText').innerText =
                `You are about to disable "${name}". This will remove it from booking options.`;

            document.getElementById('disableForm').action =
                `/superadmin/truck-types/${id}/toggle`;
