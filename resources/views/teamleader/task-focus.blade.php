...
@push('styles')
    @if (is_file($tasksCssPath))
        <style>{!! file_get_contents($tasksCssPath) !!}</style>
    @endif

    <style>
        /* ── Page wrapper ── */
        .tf-page { max-width: 860px; margin: 0 auto; padding: 0 0 60px; }

        /* ── Stepper ── */
        .tf-stepper {
            display: flex; align-items: flex-start; gap: 0;
            background: #fff; border-radius: 16px;
            border: 1px solid #e4e4e7; padding: 18px 24px;
            margin-bottom: 16px; overflow-x: auto;
        }
        .tf-step {
            display: flex; flex-direction: column; align-items: center;
            gap: 6px; flex: 1; min-width: 64px; position: relative;
        }
        .tf-step__bubble {
        ...
        .tf-step__label { text-align: center; }
        .tf-step__label strong { display: block; font-size: 11px; color: #3f3f46; }
        .tf-step__label small  { display: block; font-size: 10px; color: #a1a1aa; }
        .tf-step.is-active .tf-step__label strong { color: #09090b; }
        .tf-step.is-complete .tf-step__label strong { color: #16a34a; }
        .tf-step-line {
            flex: 1; height: 2px; background: #e4e4e7;
            margin-top: 17px; min-width: 16px;
            transition: background .2s;
        }
        .tf-step-line.is-done { background: #16a34a; }
        ...
        .tf-banner__meta { display: flex; flex-direction: column; gap: 4px; }
        .tf-banner__code { font-size: 11px; font-weight: 700; color: #a1a1aa; text-transform: uppercase; letter-spacing: .06em; }
        .tf-banner__note { font-size: 13px; color: #52525b; }
        .tf-status-pill {
            display: inline-flex; align-items: center; gap: 6px;
            padding: 6px 14px; border-radius: 999px; font-size: 12px; font-weight: 700;
            white-space: nowrap; border: 1px solid transparent;
        }
        .tf-status-pill.assigned       { background: #f0fdf4; color: #15803d; border-color: #86efac; }
        .tf-status-pill.on-the-way     { background: #eff6ff; color: #1d4ed8; border-color: #93c5fd; }
        .tf-status-pill.in-progress    { background: #fefce8; color: #a16207; border-color: #fde047; }
        .tf-status-pill.waiting-verification { background: #faf5ff; color: #7c3aed; border-color: #c4b5fd; }
        .tf-status-pill.payment-pending  { background: #fff7ed; color: #c2410c; border-color: #fdba74; }
        .tf-status-pill.payment-submitted{ background: #f0fdf4; color: #15803d; border-color: #86efac; }
        .tf-status-pill.completed        { background: #f0fdf4; color: #15803d; border-color: #86efac; }
        ...
        .tf-card__rows { display: flex; flex-direction: column; gap: 10px; }
        .tf-card__rows--horiz {
            display: grid; grid-template-columns: repeat(auto-fill, minmax(180px, 1fr)); gap: 10px;
        }
        .tf-card__row { display: flex; flex-direction: column; gap: 2px; }
        .tf-card__row--full { grid-column: 1 / -1; }
        .tf-card__label { font-size: 11px; color: #a1a1aa; font-weight: 600; }
        .tf-card__value { font-size: 13px; color: #18181b; font-weight: 500; word-break: break-word; }
        ...
        .tf-unit-badge__name { font-size: 14px; font-weight: 700; color: #09090b; }
        .tf-unit-type {
            font-size: 11px; font-weight: 700; padding: 3px 8px; border-radius: 6px;
            background: #e4e4e7; color: #3f3f46; margin-left: auto;
        }

        /* Pricing */
        .tf-price-rows { display: flex; flex-direction: column; gap: 8px; }
        .tf-price-row { display: flex; justify-content: space-between; font-size: 13px; color: #3f3f46; }
        .tf-price-divider { border: none; border-top: 1px solid #e4e4e7; margin: 6px 0; }
        .tf-price-row--total { font-size: 15px; font-weight: 800; color: #09090b; }
        .tf-price-note { font-size: 11px; color: #a1a1aa; margin: 6px 0 0; text-align: right; }
        ...
        .tf-svc-badge--now  { background: #eff6ff; color: #1d4ed8; }
        .tf-svc-badge--sched{ background: #faf5ff; color: #7c3aed; }
        .tf-mono { font-family: monospace; font-size: 13px; }
        ...
        .tf-action-btns { display: flex; flex-direction: column; gap: 10px; }
        .tf-btn {
            display: inline-flex; align-items: center; justify-content: center;
            gap: 6px; border-radius: 12px; font-size: 14px; font-weight: 700;
            padding: 13px 20px; border: none; cursor: pointer; width: 100%;
            transition: background .15s, opacity .15s;
        }
        .tf-btn:disabled { opacity: .5; cursor: not-allowed; }
        .tf-btn--primary { background: #09090b; color: #fff; }
        .tf-btn--primary:hover:not(:disabled) { background: #27272a; }
        .tf-btn--ghost   { background: #f4f4f5; color: #3f3f46; }
        .tf-btn--ghost:hover:not(:disabled)   { background: #e4e4e7; }
        ...
        .tf-feedback {
            font-size: 13px; color: #71717a; text-align: center; margin: 0;
        }
        .tf-feedback.is-error { color: #dc2626; }

        /* Waiting card */
        .tf-waiting-card {
            background: #faf5ff; border: 1px solid #c4b5fd; border-radius: 12px;
            padding: 16px; text-align: center; font-size: 13px; color: #7c3aed;
        }
        .tf-waiting-card strong { display: block; font-size: 15px; margin-bottom: 4px; }

        /* ── Payment form ── */
        .tf-payment-section { display: flex; flex-direction: column; gap: 16px; }
        .tf-payment-section h3 { margin: 0; font-size: 16px; color: #09090b; }
        .tf-payment-section p  { margin: 0; font-size: 13px; color: #52525b; }

        .tf-pm-grid { display: grid; grid-template-columns: repeat(3, 1fr); gap: 10px; }
        .tf-pm-label { cursor: pointer; }
        .tf-pm-label input[type=radio] { display: none; }
        .tf-pm-btn {
            display: flex; flex-direction: column; align-items: center; gap: 4px;
            border: 2px solid #e4e4e7; border-radius: 12px; padding: 14px 8px;
            font-size: 13px; font-weight: 600; color: #3f3f46;
            transition: border-color .15s, background .15s;
        }
        .tf-pm-label input:checked + .tf-pm-btn {
            border-color: #09090b; background: #f9fafb; color: #09090b;
        }
        .tf-pm-icon { font-size: 20px; }

        .tf-file-drop {
            border: 2px dashed #e4e4e7; border-radius: 12px; padding: 24px;
            text-align: center; cursor: pointer; transition: border-color .15s, background .15s;
            position: relative;
        }
        .tf-file-drop:hover, .tf-file-drop.is-hover { border-color: #09090b; background: #f9fafb; }
        .tf-file-drop p { margin: 0; font-size: 14px; font-weight: 600; color: #18181b; }
        .tf-file-drop small { font-size: 12px; color: #a1a1aa; }
        #proofPreview { width: 100%; max-height: 200px; object-fit: contain; border-radius: 8px; margin-top: 8px; }
        ...
        .tf-submitted-card h3 { margin: 0 0 6px; font-size: 17px; color: #14532d; }
        ...
        .tl-dialog-card { width: min(520px,100%); background: #fff; border-radius: 18px; padding: 22px; box-shadow: 0 24px 60px rgba(15,23,42,.22); }
        .tl-dialog-card h3 { margin: 6px 0 8px; }
        .tl-dialog-card p  { margin: 0 0 12px; color: #475569; }
        .tl-dialog-card select, .tl-dialog-card textarea { width: 100%; border: 1px solid #cbd5e1; border-radius: 12px; padding: 10px 12px; font: inherit; margin-top: 8px; }
        .tl-dialog-card textarea { min-height: 100px; resize: vertical; }
        .tl-dialog-actions { display: flex; justify-content: flex-end; gap: 10px; margin-top: 14px; }

        /* Toast */
        .tl-toast-wrap { position: fixed; bottom: 24px; right: 24px; display: flex; flex-direction: column; gap: 8px; z-index: 9999; }
        .tl-toast { padding: 12px 18px; border-radius: 12px; font-size: 13px; font-weight: 600; box-shadow: 0 8px 24px rgba(0,0,0,.12); animation: tl-slide-in .25s ease; }
        .tl-toast--success { background: #09090b; color: #fff; }
        .tl-toast--error   { background: #dc2626; color: #fff; }
        @keyframes tl-slide-in { from { transform: translateY(16px); opacity: 0; } to { transform: translateY(0); opacity: 1; } }
...
<div class="tf-page" id="focusedTask"
        ...
        <div class="tf-card">
            ...
            <div class="tf-card__rows">
                <div class="tf-card__row">
                    ...
                    <span class="tf-card__value">{{ $booking->customer?->full_name ?? $booking->customer?->name ?? 'Guest' }}</span>
                </div>
                <div class="tf-card__row">
        ...
        <div class="tf-card">
            ...
            <div class="tf-card__rows">
                ...
                <div class="tf-card__row">
                    ...
                    <span class="tf-card__value">{{ $distanceKm > 0 ? number_format($distanceKm, 2) . ' km' : '—' }}</span>
                </div>
            </div>
        </div>
            ...
            <div class="tf-card__rows tf-card__rows--horiz">
                <div class="tf-card__row">
                    ...
                    <span class="tf-card__value">
                        <span class="tf-svc-badge {{ $booking->service_type === 'book_now' ? 'tf-svc-badge--now' : 'tf-svc-badge--sched' }}">
                            {{ $serviceLabel }}
                        </span>
                    </span>
                </div>
    ...
    <div class="tf-action-area">
        ...
        <div class="tf-action-btns" id="focusActionGroup">
            ...
            <button type="button" class="tf-btn tf-btn--primary {{ $task['can_complete'] ? '' : 'hidden' }}" id="completeTaskBtn">
                Complete Job — Send to Customer
            </button>
            <button type="button" class="tf-btn tf-btn--ghost {{ $task['can_return'] ? '' : 'hidden' }}" id="returnTaskBtn">
                Return to Dispatch
            </button>
            <a href="{{ route('teamleader.dashboard') }}" class="tf-btn tf-btn--success hidden" id="backToDashboardBtn">
                Back to Dashboard
            </a>
        </div>

        {{-- Waiting for customer verification --}}
        <div id="waitingVerificationCard" class="tf-waiting-card {{ $currentStatus === 'waiting_verification' ? '' : 'hidden' }}">
...
<div class="tl-dialog-backdrop hidden" id="returnTaskModal" aria-hidden="true">
    <div class="tl-dialog-card">
        ...
        <select id="returnReasonPreset"><option value="">Select a reason</option></select>
