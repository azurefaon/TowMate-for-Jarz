...
Map<String, dynamic> _scheduledExtraPreview({
  ...
  final distanceFee = distanceKm > 4.0 ? double.parse(((distanceKm - 4.0) * perKmRate).toStringAsFixed(2)) : 0.0;
...
Map<String, dynamic> _pricingResponse({
  ...
  final vat = double.parse((subtotal * 0.12).toStringAsFixed(2));
  final finalTotal = double.parse((subtotal + vat + additionalFee).toStringAsFixed(2));
  final requestTotal = double.parse(
    (finalTotal + scheduledExtraPreviews.fold<double>(0, (sum, e) => sum + (e['final_total'] as num).toDouble()))
        .toStringAsFixed(2),
  );
  return {
...
void main() {
  ...
  group('BookNowScreen Step 2 pricing authoritative behavior', () {
    testWidgets('canonical distance <= 4 km shows Distance Fee of ₱0.00', (tester) async {
      await _pumpToStep2(tester, pricingOverride: _pricingResponse(distanceKm: 3.5));
    ...
    testWidgets('canonical distance > 4 km only charges the excess after 4 km', (tester) async {
      await _pumpToStep2(tester, pricingOverride: _pricingResponse(distanceKm: 6.0, perKmRate: 300));
    ...
    testWidgets('VAT is computed from the canonical taxable subtotal, matching the server value', (tester) async {
      final resp = _pricingResponse(distanceKm: 5.64, baseRate: 1500, perKmRate: 300);
    ...
    testWidgets('Review displays the server preview total, not a locally recomputed value', (tester) async {
      final resp = _pricingResponse(distanceKm: 5.64, baseRate: 1500, perKmRate: 300);
      await _pumpToStep2(tester, pricingOverride: resp);
    ...
    testWidgets('route distance and canonical pricing distance are both shown, clearly labeled, never implied equal', (
      tester,
    ) async {
      await _pumpToStep2(
        tester,
    ...
    testWidgets('Truck Type / Tow Type is never exposed on the Review screen', (tester) async {
  ...
  group('BookNowScreen Step 2 vehicle & photo display', () {
    testWidgets('Vehicle 1 name and photo are visible on Review', (tester) async {
      await _pumpToStep2(tester);
    ...
    testWidgets('an additional vehicle photo is visible on Review', (tester) async {
      final previousPlatform = ImagePickerPlatform.instance;
      ImagePickerPlatform.instance = _FakeImagePickerPlatform(_createFakePhotoFile());
      addTearDown(() => ImagePickerPlatform.instance = previousPlatform);
    ...
    testWidgets('multiple photos for a vehicle display as compact thumbnails', (tester) async {
      final previousPlatform = ImagePickerPlatform.instance;
      ...
      await http.runWithClient(() async {
        ...
        for (var i = 0; i < 2; i++) {
          await tester.tap(find.text(i == 0 ? 'Add vehicle photos' : 'Add more'));
          await _settle(tester);
          await tester.tap(find.text('Choose from Gallery'));
          await _settle(tester);
        }
    ...
    testWidgets('a Scheduled vehicle shows its confirmed date and time, not Book Now', (tester) async {
      final previousPlatform = ImagePickerPlatform.instance;
      ImagePickerPlatform.instance = _FakeImagePickerPlatform(_createFakePhotoFile());
      addTearDown(() => ImagePickerPlatform.instance = previousPlatform);
    ...
    for (final width in [320.0, 360.0, 390.0, 412.0]) {
      testWidgets('renders Step 2 with 3 vehicles without overflow at ${width.toInt()}px', (tester) async {
        final previousPlatform = ImagePickerPlatform.instance;
        ImagePickerPlatform.instance = _FakeImagePickerPlatform(_createFakePhotoFile());
        addTearDown(() => ImagePickerPlatform.instance = previousPlatform);
  ...
  group('BookNowScreen Step 2 loading, error and submit safety', () {
    testWidgets('pricing loading uses SkeletonBox, not a CircularProgressIndicator', (tester) async {
      tester.view.physicalSize = const Size(390, 844);
      tester.view.devicePixelRatio = 1.0;
      addTearDown(tester.view.resetPhysicalSize);
      addTearDown(tester.view.resetDevicePixelRatio);
    ...
    testWidgets('pricing failure shows a clear retry state, not stale/local pricing', (tester) async {
    ...
    testWidgets('Confirm Booking is disabled while pricing is unresolved (failed)', (tester) async {
      await _pumpToStep2(tester, pricingFails: true);
    ...
    testWidgets('Confirm Booking becomes enabled once pricing resolves successfully', (tester) async {
      await _pumpToStep2(tester);
    ...
    testWidgets('tapping Confirm Booking twice does not trigger a double submit', (tester) async {
      var submitCount = 0;
      tester.view.physicalSize = const Size(390, 844);
      tester.view.devicePixelRatio = 1.0;
      addTearDown(tester.view.resetPhysicalSize);
      ...
      final client = MockClient((request) async {
        ...
        if (path.endsWith('/v1/bookings') && request.method == 'POST') {
          ...
          return _json({'success': false, 'message': 'Blocked for test'}, status: 422);
        }
        return _json({}, status: 404);
      });
    ...
    testWidgets('a successful submission navigates to the Booking Request Submitted screen, not straight Home', (
      tester,
    ) async {
      tester.view.physicalSize = const Size(390, 844);
      tester.view.devicePixelRatio = 1.0;
      ...
      final client = MockClient((request) async {
        final url = request.url.toString();
        if (url.contains('router.project-osrm.org')) return _json({'code': 'Error'});
        final path = request.url.path;
        if (path.contains('vehicle-types')) return _json(_vehicleTypesPayload());
        if (path.contains('availability')) {
          return _json({
      ...
      expect((capturedArgs as List<BookingGroupSibling>).single.bookingCode, 'TM-00225');
    });
  });

  group('BookNowScreen Step 2 single-mode review & pricing (Task D)', () {
    testWidgets('Book Now only: shows Total, no SCHEDULED section', (tester) async {
    ...
    testWidgets('Edit beside TRIP returns to Step 0 preserving pickup/drop-off coordinates', (tester) async {
      await _pumpToStep2(tester, pricingOverride: _pricingResponse());
    ...
    testWidgets('Edit beside VEHICLES returns to Step 1 preserving vehicle selection', (tester) async {
      await _pumpToStep2(tester, pricingOverride: _pricingResponse());
    ...
    testWidgets('Vehicle 1 scheduled alone (no extras): Estimated Request Total is not shown', (tester) async {
      SharedPreferences.setMockInitialValues({'auth_token': 'test-token', 'user_role': 'Customer'});
      final previousPlatform = ImagePickerPlatform.instance;
      ImagePickerPlatform.instance = _FakeImagePickerPlatform(_createFakePhotoFile());
      addTearDown(() => ImagePickerPlatform.instance = previousPlatform);
      ...
      await http.runWithClient(() async {
        ...
        await tester.tap(find.text('Continue'));
        await _settle(tester);
      }, () => _client(readyTruckTypeIds: const [2], pricingOverride: _pricingResponse()));
    ...
    testWidgets('an all-Scheduled group renders every vehicle uniformly under one Price Summary', (tester) async {
      final scheduledExtra = _scheduledExtraPreview(vehicleTypeId: 3, truckTypeId: 2, baseRate: 900);
      ...
      expect(find.text('VEHICLE 1'), findsOneWidget);
      expect(find.text('VEHICLE 2'), findsOneWidget);
      expect(find.text('Estimated Base Rate'), findsNWidgets(2));
      expect(find.text('Estimated Distance Fee'), findsNWidgets(2));
      expect(find.text('Estimated VAT'), findsNWidgets(2));
      expect(find.text('Estimated Total'), findsNWidgets(2));
      expect(find.text('Total'), findsNothing);
      expect(find.text('Base Rate'), findsNothing);
      expect(
        find.textContaining('Scheduled pricing may change after quotation review.'),
        findsOneWidget,
      );
      expect(
        find.textContaining('Each vehicle will be quoted and billed separately.'),
        findsOneWidget,
      );
      expect(
    ...
    testWidgets('multiple Scheduled extras in an all-Scheduled group each render their own Estimated rows', (
      tester,
    ) async {
      final extra1 = _scheduledExtraPreview(vehicleTypeId: 3, truckTypeId: 2, baseRate: 900);
      final extra2 = _scheduledExtraPreview(vehicleTypeId: 4, truckTypeId: 2, baseRate: 700);
      ...
      Future<void> addScheduledExtra(String vehicleName) async {
        ...
        await tester.tap(find.text('Add another vehicle'));
        await _settle(tester);
        await tester.ensureVisible(find.text('Browse all vehicle types').last);
        await tester.tap(find.text('Browse all vehicle types').last);
        await _settle(tester);
      ...
      expect(find.text('VEHICLE 2'), findsOneWidget);
      expect(find.text('VEHICLE 3'), findsOneWidget);
      expect(find.text('Estimated Base Rate'), findsNWidgets(3));
      expect(find.text('Estimated Distance Fee'), findsNWidgets(3));
      expect(find.text('Estimated VAT'), findsNWidgets(3));
      expect(find.text('Estimated Total'), findsNWidgets(3));

      await tester.pumpWidget(const SizedBox());
    });

    testWidgets('request_total is never sent as part of the booking submission payload', (tester) async {
      String? capturedBody;
      final scheduledExtra = _scheduledExtraPreview(vehicleTypeId: 3, truckTypeId: 2, baseRate: 900);
      final resp = _pricingResponse(scheduledExtraPreviews: [scheduledExtra]);
    ...
    for (final width in [320.0, 360.0, 390.0, 412.0]) {
      testWidgets('all-Scheduled group Review renders without overflow at ${width.toInt()}px', (tester) async {
        final scheduledExtra = _scheduledExtraPreview(vehicleTypeId: 3, truckTypeId: 2, baseRate: 900);
        final resp = _pricingResponse(scheduledExtraPreviews: [scheduledExtra]);
